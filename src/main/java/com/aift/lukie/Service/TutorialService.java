package com.aift.lukie.Service;
import java.util.ArrayList;
import java.util.List;

import org.springframework.stereotype.Service;

import com.aift.lukie.Model.Tutorial;
...
public class TutorialService {
  ...
  public List<Tutorial> findByTitleContaining(String title) {
    return (List<Tutorial>) tutorials.stream().filter(tutorial -> tutorial.getTitle().contains(title));
  }
  ...
  public List<Tutorial> findByPublished(boolean isPublished) {
    return ((List<Tutorial>) tutorials.stream().filter(tutorial -> isPublished == tutorial.isPublished()));
  }
}
